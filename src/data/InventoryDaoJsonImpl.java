...
public class InventoryDaoJsonImpl implements InventoryDAO {
	...
	@PostConstruct
	void init() {
		String file = wac.getServletContext().getRealPath("WEB-INF/RecordBin-data.json");
		
		// backup the JSON file
		String bkpFile = file + Instant.now().getEpochSecond();
		
		try {
			Files.copy(Paths.get(file), Paths.get(bkpFile), StandardCopyOption.REPLACE_EXISTING);
		} catch (IOException e) {
			e.printStackTrace();
		}
		
		setJsonFile(file); // calls loadFromFile()
	}
	...
	public void destroyAlbum(Album album) {
		if(album == null)
			return;
